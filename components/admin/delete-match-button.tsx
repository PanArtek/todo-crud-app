...
export function DeleteMatchButton({ matchId, homeTeam, awayTeam }: DeleteMatchButtonProps) {
  ...
  const handleDelete = async () => {
    ...
    if (result.success) {
      ...
      setError(result.error || "Nie udalo sie usunac meczu");
      setIsDeleting(false);
    }
  };
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-xl bg-[var(--bg-card)] p-6 shadow-xl">
            ...
            <h3 className="text-lg font-semibold text-white">Usunac mecz?</h3>
            <p className="mt-2 text-[var(--text-secondary)]">
              Czy na pewno chcesz usunac mecz <strong className="text-white">{homeTeam}</strong> vs{" "}
              <strong className="text-white">{awayTeam}</strong>? Ta operacja jest nieodwracalna.
            </p>
            {error && (
